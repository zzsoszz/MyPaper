...
function show_vul(oid,id){
    var $oidDiv = jQuery('#'+oid);
    var posY = 0;
    if($oidDiv.length){
        jQuery('#'+oid).removeClass('plus');
        jQuery('#'+oid).addClass('minus');
    
        $('#catalog .report_content').hide();
        var $tb = jQuery('#'+id);
        $tb.css('display','');
        if(!$('#catalog').hasClass('fixed_top')){
            posY = $tb.offset().top-60-36;
        }else{
            posY = $tb.offset().top-60;
        }
...
function jumpToHash(hash){
    ...
    var $tg = jQuery(hash);
    if(!$('#catalog').hasClass('fixed_top')){
        posY = $tg.offset().top-38-36;
    }else{
        posY = $tg.offset().top-38;
    }
    $('html,body').animate({scrollTop: posY}, 500);
}
