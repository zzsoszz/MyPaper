...
function LoginUserName() {
    return getCookie("activeUserName");
}
...
function GetDisplayUserName() {
    var activeUserName = getCookie("activeUserName");
...
//拼接一条评论正文Html
function BuildContent(title, sourceUrl, id, author, dateCreated, ip, body, parentId, isExpert) {
    var userLink;
    ...
    return commentItem.format(title, sourceUrl, id, author, dateCreated, ip, Emotion2Html(body), userLink, isExpert ? expertIcon : "", parentId > 0 ? "ask" : "question", parentId > 0 ? "askauthor" : "quesauthor");
...
function LoadFeedback(entryId, userName) {
    ...
    $.getJSON(t_CommentHandler.format(queryString), function(data) {
        $.each(data.Items, function(i, item) {
            if (item.parentId <= 0) {
                var html = BuildContent(item._title, item._sourceurl, item._id, item._author, JsonDateTime2String(item._datecreated), item.ipAddress, item._body, item.parentId, InList(item._author, data.Experts));
