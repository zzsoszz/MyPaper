...
// Images Directory
imagesDir = "/webpart/editor/icons/";

// CSS Directory
cssDir = "/webpart/editor/styles/";
editContentCssPath=cssDir+"editContent.css";
// Popups Directory
popupsDir = "/webpart/editor/popups/";
...
var buttonName2 = new Array();
  buttonName2[0]  = "forecolor";
    ...
    buttonName2[16]  = "blockquote";	
	buttonName2[17]  = "seperator";
    buttonName2[18]  = "viewSource";  
    buttonName2[19]  = "help";
	
// List of available actions and their respective ID and images
var ToolbarList = {
//Name              buttonID                 buttonTitle           buttonImage                            buttonImageRollover
  "bold":           ['Bold',                 '黑体',               imagesDir + 'bold.gif',               imagesDir + 'bold_on.gif'],
  "italic":         ['Italic',               '斜体',             imagesDir + 'italics.gif',            imagesDir + 'italics_on.gif'],
  "underline":      ['Underline',            '下划线',          imagesDir + 'underline.gif',          imagesDir + 'underline_on.gif'],
	"strikethrough":  ['Strikethrough',        '删除线',      imagesDir + 'strikethrough.gif',      imagesDir + 'strikethrough_on.gif'],
	"seperator":      ['',                     '',                   imagesDir + 'seperator.gif',          imagesDir + 'seperator.gif'],
	"subscript":      ['Subscript',            '下标',          imagesDir + 'subscript.gif',          imagesDir + 'subscript_on.gif'],
	"superscript":    ['Superscript',          '上标',        imagesDir + 'superscript.gif',        imagesDir + 'superscript_on.gif'],
	"justifyleft":    ['Justifyleft',          '左对齐',        imagesDir + 'justify_left.gif',       imagesDir + 'justify_left_on.gif'],
	"justifycenter":  ['Justifycenter',        '居中对齐',      imagesDir + 'justify_center.gif',     imagesDir + 'justify_center_on.gif'],
	"justifyright":   ['Justifyright',         '右对齐',       imagesDir + 'justify_right.gif',      imagesDir + 'justify_right_on.gif'],
	"unorderedlist":  ['InsertUnorderedList',  '无序列表',imagesDir + 'list_unordered.gif',     imagesDir + 'list_unordered_on.gif'],
	...
	"copy":           ['Copy',                 '复制',               imagesDir + 'copy.gif',               imagesDir + 'copy_on.gif'],
    "paste":          ['Paste',                '粘贴',              imagesDir + 'paste.gif',              imagesDir + 'paste_on.gif'],
	"forecolor":      ['ForeColor',            '选择文字颜色',          imagesDir + 'forecolor.gif',          imagesDir + 'forecolor_on.gif'],
	"backcolor":      ['BackColor',            '选择背景颜色',          imagesDir + 'backcolor.gif',          imagesDir + 'backcolor_on.gif'],
	"undo":           ['Undo',                 '撤销',               imagesDir + 'undo.gif',               imagesDir + 'undo_on.gif'],
	...
	"insertimage":    ['InsertImage',          '插入图片',        imagesDir + 'insert_picture.gif',     imagesDir + 'insert_picture_on.gif'],
	"createlink":     ['CreateLink',           '创建链接',         imagesDir + 'insert_hyperlink.gif',   imagesDir + 'insert_hyperlink_on.gif'],
	"viewSource":     ['ViewSource',           '查看源代码',         imagesDir + 'view_source.gif',        imagesDir + 'view_source.gif'],
	"viewText":       ['ViewText',             '正常编辑',           imagesDir + 'view_text.gif',          imagesDir + 'view_text.gif'],
	"help":           ['Help',                 '帮助',               imagesDir + 'help.gif',               imagesDir + 'help_on.gif'],
	"selectfont":     ['SelectFont',           '选择字体',         imagesDir + 'select_font.gif',        imagesDir + 'select_font_on.gif'],
	"selectsize":     ['SelectSize',           '选择字号',         imagesDir + 'select_size.gif',        imagesDir + 'select_size_on.gif'],
	"multimedia":  ['MultiMedia',        '插入电影、音乐',      imagesDir + 'media.gif',        imagesDir + 'media.gif'],
	"insertsmileys":  ['InsertSmileys',        '插入表情',      imagesDir + 'smileys.gif',        imagesDir + 'smileys.gif'],
	"blockquote":  ['BlockQuote',        '引用',      imagesDir + 'blockquote.gif',        imagesDir + 'blockquote.gif']
};
...
/* ---------------------------------------------------------------------- *\
  ...
  Usage       : editor_generate("textarea_id");
  Arguments   : textarea_id - ID of textarea to replace
\* ---------------------------------------------------------------------- */
function editor_generate(textareaID) {
 	//set height & width
 	
 	wysiwygWidth = document.getElementById(textareaID).clientWidth;
	wysiwygHeight = document.getElementById(textareaID).clientHeight;
  	// Hide the textarea 
  ...
  toolbar =  '<table cellpadding="0" cellspacing="0" border="0" class="toolbar1" style="width:' + toolbarWidth + 'px;"><tr><td style="width: 6px;"><img src="' +imagesDir+ 'seperator2.gif" alt="" hspace="3"></td>';
    ...
    if (buttonName[i]) {
      ...
      var buttonImage          = buttonObj[2];
		    ...
		    toolbar += '<td style="width: 22px;"><img src="' +buttonImage+ '" border=0 unselectable="on" title="' +buttonTitle+ '" id="' +buttonID+ '" class="button" onClick="formatText(this.id,\'' + n + '\');" onmouseover="if(className==\'button\'){className=\'buttonOver\'};" onmouseout="if(className==\'buttonOver\'){className=\'button\'};" unselectable="on" width="20" height="20"></td>';
	    }
    }
    i++;
  }
  ...
  toolbar2 += '<td>&nbsp;</td></tr></table>';  
	...
	var iframe = '<table class="iframeBox" cellpadding="0" cellspacing="0" style="width:' + wysiwygWidth + 'px; height:' + wysiwygHeight + 'px;"><tbody><tr><td valign="top">'
  + '<iframe class="iframe" id="wysiwyg' + n + '" name="wysiwyg' + n + '" src="/webpart/editor/blank.html?t&editor='+n+'&height='+wysiwygHeight+'" onload="setDesignMode(this);"></iframe>'
  + '</td></tr></tbody></table>';
  //src="/webpart/editor/blank.html"
  // Insert after the textArea both toolbar one and two
  document.getElementById(n).insertAdjacentHTML("afterEnd", "<div class=\"wysiwyg\">"+toolbar + toolbar2 + iframe+"</div>");
  ...
  if (navigator.appName == "Microsoft Internet Explorer") {
  	doc.write("<html><head><link rel=\"stylesheet\" type=\"text/css\" href=\""+editContentCssPath+"\" media=\"screen\" /></head><body>"+content+"</body></html>");
  }else{
  	doc.write(content);
  }  
  if(doc.createStyleSheet=="function"){
  	doc.createStyleSheet(editContentCssPath);
  }
  doc.close();
...
function formatText(id, n, selected) {
  ...
  document.getElementById("wysiwyg" + n).contentWindow.focus();
	...
	if (viewTextMode == 1 && disabled_id == 1) {
      ...
      window.open(popupsDir + 'insert_image.html?wysiwyg=' + n,'popup','location=0,status=0,scrollbars=1,resizable=1,width=750,height=550');
	  }
	    ...
	    window.open(popupsDir + 'about.html','popup','location=0,status=0,scrollbars=0,resizable=0,width=400,height=330');
	  	...
	  	window.open(popupsDir + 'media.html?wysiwyg=' + n,'popup','location=0,status=0,scrollbars=0,resizable=0,width=400,height=330');
	  }
	  else if(id=="InsertSmileys"){
	  	window.open(popupsDir + 'insert_smileys.html?wysiwyg=' + n,'popup','location=0,status=0,scrollbars=1,resizable=1,width=700,height=560');
	  }
	  else if(id=="BlockQuote"){
	  	var d=document.getElementById("wysiwyg" + n).contentWindow.document;
	  	var sel=editor_getSelection(n);	
	  	var node=d.createElement("blockquote");
	  	var range="";
	  	if(!isMSIE){
	  		...
	  		insertHTML("<blockquote>"+html+"</blockquote>&nbsp;",n);
	  	}
	  }
		// Every other command
	  else {
...
function insertHTML(html, n) {
  ...
  var browserName = navigator.appName;
	...
	if (browserName == "Microsoft Internet Explorer") {	  
		...
		div.innerHTML = html;
		var node = insertNodeAtSelection(doc,div, n);
	}
...
\* ---------------------------------------------------------------------- */
function insertNodeAtSelection(doc,insertNode, n) {
...
function outputFontSizes(n) {
  ...
  FontSizesDropDown[n] += '<span id="Sizes' + n + '" class="dropdown" style="width: 170px;">';
	...
	for (var i = 0; i <= FontSizes.length;) {
	  if (FontSizes[i]) {
      FontSizesDropDown[n] += '<button type="button" onClick="formatText(\'FontSize\',\'' + n + '\',\'' + FontSizes[i] + '\')\;hideFontSizes(\'' + n + '\');" onMouseOver="this.className=\'mouseOver\'" onMouseOut="this.className=\'mouseOut\'" class="mouseOut" style="width: 145px;"><table cellpadding="0" cellspacing="0" border="0"><tr><td align="left" style="font-family: arial, verdana, helvetica;">size ' + FontSizes[i] + '</td></tr></table></button><br>';	
    }	  
	  i++;
  }
...
function editor_getHTML(textarea_id) {
	try{
		/*
	if(viewTextMode==1){
		viewText(textarea_id);
	...
	*/	
	return updateTextArea(textarea_id);
	}catch(e){
		return document.getElementById(textarea_id).value;
	}
};
function setDesignMode(t){
	//var id=t.id;
	document.domain=UUZONE_DOMAIN;
	//document.getElementById(id).contentWindow.document.body.innerHTML = 'On';
};
function editor_setHTML(textarea_id, value){
	try{
		document.getElementById(textarea_id).value=value;
		document.getElementById('wysiwyg' + textarea_id).contentWindow.document.body.innerHTML=value;
	}catch(e){}
};
function editor_getSelection(textarea_id){
	var w=document.getElementById("wysiwyg" + textarea_id).contentWindow;
	var d=w.document;
	var s= '';
    if (window.getSelection){ s= w.getSelection();
    }else if (document.getSelection){ s= d.getSelection();
    }else if (document.selection) {s= d.selection;} //d.selection.createRange().htmlText;
    return s;
}