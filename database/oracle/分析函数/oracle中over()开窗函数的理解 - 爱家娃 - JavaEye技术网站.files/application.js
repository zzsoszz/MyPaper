...
Validator.bad_words=[/fuck/i,/shit/i,/QQ群/i,/MSN群/i,/求(助|救)/,/(跪|裸)求/,/急急|我很急|紧急寻求/,/在线等/,/救(命|我|急|救)/,/急(!|！)/,/(高手|高人|大虾|大侠|达人|前辈|快来|没有人)(指点|解答|帮忙|帮帮|指教|赐教|提点|不吝赐教|到哪)/,/(我是|帮帮)(新手|小弟|菜鸟)/,/我顶|顶上去|好贴要顶|帮顶|看看先|顶一下/,/有*没有人(回答|帮忙|关注|知道啊|回复)/,/教主|装B|装13/,/图书兼职作者/];
Validator.prototype={
...
if(submit_button){
Element.insert(submit_button,{"after":$('_form_spinner_').remove()});
$('_form_spinner_').show();
}
}
},
...
if(!$A(['jpg','bmp','png','gif','rar','zip','tar','gz','jar','war', 'bz2']).any(function(extName){return new RegExp('\\.'+extName+'$','i').test(ele.value);})){
...
$$('#n'+news_id+' a').each(function(a){a.onclick="return false;"});
new Ajax.Request('/news/digg',{asynchronous:true,evalScripts:true,parameters:{"id":news_id}});
}
function bury_news(news_id){
$$('#n'+news_id+' a').each(function(a){a.onclick="return false;"});
new Ajax.Request('/news/bury',{asynchronous:true,evalScripts:true,parameters:{"id":news_id}});
}
function digg_blog(blog_id){
$$('#b'+blog_id+' a').each(function(a){a.onclick="return false;"});
new Ajax.Request('/blogs/digg',{asynchronous:true,evalScripts:true,parameters:{"id":blog_id}});
}
