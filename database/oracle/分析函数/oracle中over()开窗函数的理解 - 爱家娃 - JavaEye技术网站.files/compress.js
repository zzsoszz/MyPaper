...
this._addButton("Code",function(){this.wrapSelection('[code="java"]','[/code]');},function(){this.innerHTML='代码: [code="ruby"]代码[/code] (支持java, ruby, js, xml, html, python, c, c++, c#, sql)';});
this._addButton("List",function(){this.insertBeforeEachSelectedLine('[*]','[list]\n','\n[/list]')},function(){this.innerHTML='列表: [list] [*]文字 [*]文字 [*]文字 [/list] (alt+l)';});
this._addButton("List=",function(){this.insertBeforeEachSelectedLine('[*]','[list=1]\n','\n[/list]')},function(){this.innerHTML='顺序列表: [list=1] [*]文字  [/list] 或者 [list=A] [*]文字 [/list] (alt+o)';});
...
tinyMCE.init({
plugins:"javaeye,media,table,emotions,contextmenu,fullscreen",
mode:"none",
language:"zh",
theme:"advanced",
...
$('editor_auto_save_update').update(this.auto_save.updated_at+'自动保存了一份草稿，您要<a href="#" onclick=\'editor._setAutosave();return false;\'>恢复</a>它，还是<a href="#" onclick=\'editor._discardAutosave();return false;\'>丢弃</a>呢？');
}
},
...
$('editor_auto_save_update').update('自动保存于：'+new Date().toLocaleString());
}
});
}
});